...
import { Clock, ArrowLeft, ArrowRight, CreditCard, CheckCircle2 } from 'lucide-react';
import { useCart } from '@/context/CartContext';
import { siteConfig } from '@/site.config';
import { getStoreStatus, fullHours } from '@/data/hours';
import Image from 'next/image';

type Step = 'review' | 'pickup' | 'payment' | 'success';
      ...
      <section className="bg-bg-alt border-b border-surface-muted">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-16">
          ...
          <div className="mt-6 flex items-center gap-2 text-xs">
            {(['review', 'pickup', 'payment'] as const).map((s, i) => (
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
                ...
                {items.map((item) => (
                  ...
                  >
                    <div className="relative h-16 w-16 rounded-lg overflow-hidden shrink-0">
                      <Image
                        src={`/menu-items/${item.id === 'espresso' ? '1' : item.id === 'cortado' ? '2' : item.id === 'oat-latte' ? '3' : item.id === 'cold-brew' ? '4' : item.id === 'pour-over' ? '5' : item.id === 'matcha-latte' ? '6' : item.id === 'chai' ? '7' : item.id === 'fresh-juice' ? '8' : item.id === 'croissant' ? '9' : item.id === 'avocado-toast' ? '10' : item.id === 'granola-bowl' ? '11' : item.id === 'sandwich' ? '12' : item.id === 'lavender-latte' ? '13' : item.id === 'mango-matcha' ? '14' : '15'}.jpg`}
                        alt={item.name}
                        fill
          ...
          {step === 'payment' && (
            <div className="space-y-6">
              ...
              {/* Payment placeholder — replace with Razorpay in production */}
              <div className="bg-surface rounded-xl border border-surface-muted p-5">
                ...
                <p className="text-xs text-text-muted mb-4">
                  In production, Razorpay&apos;s payment form will appear here with UPI, cards, and wallets.
